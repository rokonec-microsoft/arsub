    ...
    class Program
    {
        static int Main(string[] args)
        {
            return Parser.Default.ParseArguments(args, GetOptions())
                .MapResult((ICommandLineOptions opts) => RunOperation(opts),
                    (errs => 1));
        }
        ...
        private static int RunOperation(ICommandLineOptions opts)
        {
            try
            {
        ...
        {
            // This order will mandate the order in which the commands are displayed if typing just 'arsub'
            // so keep these sorted.
            return new Type[]
                {
                    typeof(ConfigOptions),
                    //typeof(ImportIssueRoutingOptions),
                    typeof(SubscribeToExistingIssuesOptions),
                    typeof(UnsubscribeToExistingIssuesOptions),
                };
        }
    }
}
